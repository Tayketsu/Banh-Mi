...
#### 2.2 Gamification
In order to adress the issues *eco-friendlyness* and *user behaviour*, we implemented a gamification-based approach. The applications keeps track of the user's data and visualizes it. Based on the distance travelled by the electric vehicles an estimate of avoided carbon dioxide compared to fuel-powered cars can be calculated. Additionally, based on the fact that a medium-sized tree binds averagely 6 kgs of carbon dioxide per year, the value is converted into an equivalent of how many trees bind this avoided amount of emmisions. 
# Heading
