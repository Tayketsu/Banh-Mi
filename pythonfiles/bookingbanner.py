...
class bookingBanner(GridLayout):

    def on_touch_down(self, touch):
        # Make it so if there's a double tap and it's a friend booking banner (not your own!) it registers a like
        if self.likeable:
            if touch.is_double_tap:
                if self.collide_point(*touch.pos):
                    ...
                    app.my_firebase.update_likes(their_friend_id, self.booking_key, likes)


    def __init__(self, **kwargs):
        self.rows = 1
        try:
            # Pass kwarg likeable=True to a bookingBanner to enable double-tap-to-like
            self.likeable = kwargs['likeable']
            self.booking_key = kwargs['booking_key']
        except:
            self.likeable = False
            self.booking_key = ""

        super(bookingBanner, self).__init__(**kwargs)
        ...
        left = FloatLayout()
        left_image = Image(source="icons/bookings/" + kwargs['booking_image'], size_hint=(1, 0.5), pos_hint={"top": .75, "right": 1})
